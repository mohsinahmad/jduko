...
class Link extends MY_Controller
{
    ...
    public function execute_query(){
            ...
            $this->db->set('AccountId','1166');
            $this->db->where('AccountId','1163');
            $this->db->where('LevelId','50');
            $this->db->update('chart_of_account');

                if($this->db->affected_rows() > 0){


            $this->db->set('AccountId','1166');
            $this->db->where('vouchertype','BR');
            $this->db->where('LevelId','50');
            $this->db->where('permanent_vouchernumber','00004');
            $this->db->where('AccountId','918');
            if($this->db->update('archived_transactions')){
...
// exit();
    ...
    public function EditLink($id)
    {
        ...
        // print_r($max_seprate_series);
        $data = array('account_id'=>$check[0]->AccountId,'maxSeparate_Series' => $max_seprate_series[0]->Separate_Series, 'levelname' => $check[0]->LevelName,"accountname" => $check[0]->AccountName, "opening" => $check[0]->OpeningBalance, "Separate_Series" => $check[0]->Separate_Series, "active" => $check[0]->Active);
        echo json_encode($data);
    }
    public function updatelink($id,$Obalance,$Active,$Seprate,$close_bal)
    {
        // $close = $_POST['close'];
        $str  = $Obalance;
        $strlen = strlen($str);
