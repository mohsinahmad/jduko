...
class WeeklyReport extends MY_Controller
{
    ...
    public function DepartmentReport($to,$from,$t_type,$serial,$is_zero)
    {
        ...
        foreach ($levels as $key => $level) {
            ...
            $data['Maddat_Name'] = $this->MaddatModel->Get_Maddat_Name($report_Id);
            $data['IncTran'][$key]['ClosingBalance'] = 
            $this->MaddatModel->Get_Maddad_Acc_transactions($level->Id,$to,$from,$report_Id,$t_type);
            ...
            $data['IncTran'][$key]['Difference'] = 
            $data['IncTran'][$key][0][0]->Debit - $data['IncTran'][$key][0][0]->Credit;

            $data['IncTran'][$key]['BankAddition'] = 
            $data['IncTran'][$key]['ClosingBalance'][0] + $data['IncTran'][$key]['Difference'];
           // $res = $this->MaddatModel->update_balances($level->Id,$data['IncTran'][$key]['BankAddition'],$report_Id);
        }
