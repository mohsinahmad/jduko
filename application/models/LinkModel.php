...
Class LinkModel extends CI_Model
{
                ...
                if($match->num_rows() > 0){
                    ...
                    if ($data1[0]->Category == 2) {
                        ...
                        if($this->yearState->Active == 1){
                            ...
                            if($coaYear){
                              $data = array('ChartOfAccountId' => $last_Id, 'Year' => $QmYear, 'OpeningBalance' => '0.00', 'CurrentBalance' => '0.00', 'Active' => $active,'CreatedBy' => $createdBy, 'CreatedOn' => $createdOn, 'Year' => $this->ActiveYear );
    ...
    public function getdata()
    {
        ...
        if($this->yearState->Active == 1){
            ...
            $this->db->join('archived_chart_of_account_years','chart_of_account.id = archived_chart_of_account_years.ChartOfAccountId','left');
            // $this->db->where('archived_chart_of_account_years.Year', $this->ActiveYear);
        }
        if ($company != ''){
            $this->db->where('chart_of_account.LevelId',$company);
    ...
    public function  update_previous_link($id,$Obalance,$Active,$Seprate){
        $where = 'Permanent_VoucherNumber is not null';
        if(isset($_POST['close'])){
            $this->db->select('sum(debit) as total_debit, sum(credit) as total_credit');
            $this->db->where('AccountID',$_POST['account_id']);
            $this->db->where('is_delete','0');
            $this->db->where($where);
            $result = $this->db->get('archived_transactions')->result();
        }
        if(isset($Obalance)){
            $new_balance = $Obalance + $result[0]->total_debit - $result[0]->total_credit;
            $this->db->set('OpeningBalance', $Obalance);
            $this->db->set('CurrentBalance', $new_balance);
        }
        if(isset($_POST['close'])){
            $this->db->where('ChartOfAccountId',$id);
            $this->db->update('archived_chart_of_account_years');
            if($this->db->affected_rows()>0){
                $this->db->set('OpeningBalance', $new_balance);
                $this->db->set('CurrentBalance', $new_balance);
                $this->db->where('ChartOfAccountId',$id);
    ...
    // ye function again call ok active funtion ye hy update_previous_link sirf vbalance update karny k lie
    //bana hy js ki file me is ki link me method name change hoga or controller me bhi method change hoga or
    // ye ok ho jyga
    ...
    public function Get_OpeningBalance($date,$chart_id)
    {
        ...
        if($this->yearState->Active == 1){
            ...
            $this->db->where_in('archived_chart_of_account_years.ChartOfAccountId',$chart_id);
        }
    ...
    public function SetBalancesForClosing()
    {
        ...
        foreach ($companies as $key => $company) {
            ...
            $Income_Links_id =  $this->ChartModel->getIncomeAcc();
            $Expense_Links_id =  $this->ChartModel->getExpenseAcc();
