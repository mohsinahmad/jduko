...
// print_r($maddats);
 ...
 //            print_r($ChartOfaccountIds);
            ...
            // $hijri_date = $this->BookModel->get_hijri_date($from);
...
// if($closingBalance != array()){
    
// }
                    $closingBalance[$M_key] = $OpeningBal[$M_key]->OpeningBalance + $trans[$M_key][0]->Debit - $trans[$M_key][0]->Credit;
              
                }
                // echo $this->db->last_query().'if';
            }
            else{
                foreach ($maddat_accounts as $M_key => $maddat_account) {
                    $OpeningBal[$M_key] = $this->BookModel->getOpeningBal($from,$level_id,$accountIds[$M_key]);
                    $this->db->select('IFNULL(SUM(Debit),0) as Debit, IFNULL(SUM(Credit),0)  as Credit');
                    $this->db->from('archived_transactions');
                    $this->db->where('archived_transactions.LinkID', $accountIds[$M_key]);
                    //$this->db->where('transactions.LevelID', $level_id);
                    if ($trans_of == 'p') {
                    ...
                    $this->db->where('archived_income.LinkID', $accountIds[$M_key]);
                    //$this->db->where('income.LevelID', $level_id);
                    if ($trans_of == 'p') {
                    ...
                    $trans[$M_key] = $query->result();
                    $closingBalance[$M_key] = $OpeningBal[$M_key]->OpeningBalance + $trans[$M_key][0]->Debit - $trans[$M_key][0]->Credit;
                // $this->db->where('ChartOfAccountId',$accountIds[$M_key]);
                // $this->db->set('OpeningBalance',$closingBalance[$M_key]);
                // $this->db->set('CurrentBalance',$closingBalance[$M_key]);
                // $this->db->update('chart_of_account_years');
                }
                // echo $this->db->last_query().'else';
            }
           // }
        }
        // exit();
...
public function update_balances($level_id,$balance,$report_id){
 $NoAccount = 0;
        $closingBalance = array();
        $maddats = $this->Get_Maddat_Name($report_id);
        if($maddats != array()){
            ...
            foreach ($maddat_accounts as $key => $value) {     

                $this->db->set('OpeningBalance',$balance);
                $this->db->set('CurrentBalance',$balance);
                $this->db->where('ChartOfAccountId',$accountIds[$key]);
                $this->db->update('chart_of_account_years');
    ...
    public function Get_weekly_1($to,$from,$levelid,$report_Id,$previous,$trans_of,$IsReversed='')
    {
        ...
        foreach ($maddat_accounts as $N_key => $maddat_account) {
            foreach ($maddat_account as $Z_key => $acc_code) {
                if ($previous == 1) {
                    $transdata['OpeningBal'][$N_key] = $this->BookModel->getOpeningBal($from,$levelid,$OpeningBal[$N_key]);
                }
        ...
        foreach ($maddat_accounts as $N_key => $maddat_account) {
            foreach ($maddat_account as $A_key => $acc_code_id) {
                if($accountIds[$N_key][$A_key][0]->AccountCode[0].$accountIds[$N_key][$A_key][0]->AccountCode[1].$accountIds[$N_key][$A_key][0]->AccountCode[2].$accountIds[$N_key][$A_key][0]->AccountCode[3].$accountIds[$N_key][$A_key][0]->AccountCode[4] == 10202){
                    if ($previous == 1) {
                        $transdata['OpeningBal']['Captial'][$N_key] = $this->BookModel->getOpeningBal($from,$levelid,$OpeningBal['Captial'][$N_key]);
                    }
                }else{
                    if ($previous == 1) {
                        $transdata['OpeningBal']['Other'][$N_key] = $this->BookModel->getOpeningBal($from,$levelid,$OpeningBal['Other'][$N_key]);
                    }
                }
                if($accountIds[$N_key][$A_key][0]->AccountCode[0].$accountIds[$N_key][$A_key][0]->AccountCode[1].$accountIds[$N_key][$A_key][0]->AccountCode[2].$accountIds[$N_key][$A_key][0]->AccountCode[3].$accountIds[$N_key][$A_key][0]->AccountCode[4] == 10202){
