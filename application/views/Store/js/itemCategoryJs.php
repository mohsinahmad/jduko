<script type="text/javascript">
	...
	$('.getid').on('click',function(){
		id = $(this).data('id');
        $('input[name="SubCategoryName"]').val($('#edit_sub_cat option:selected').text());
        $('input[name="CategoryName"]').val($('#edit_cat option:selected').text());
		$.ajax({
			type: 'POST',
            ...
            success:function (response) {
            	...
            	$('#Item').val(data.Name);
                $('#edit_sub_cat')
                .append('<option value='+data.sub_cat_id+' selected>'+data.sub_cat_name+'</option>');
                 $('#edit_cat').append('<option value='+data.category_id+' selected>'+data.category_name+'</option>');
            }
		});
	});
    $('.update_category').on('click',function(){
        var post = new Object();
        post.Code = $('#Code').val();
        post.category = $('#edit_cat').val();
        post.sub_category = $('#edit_sub_cat').val();
        post.Name = $('#Item').val();
        post.SubCategoryName = $('input[name="SubCategoryName"]').val();
        post.CategoryName = $('input[name="CategoryName"]').val();
         $.ajax({
             type: 'POST',
