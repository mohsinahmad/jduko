<script type="text/javascript">
    ...
    $('.linkupdate').on('click',function(){
        ...
        }if($('#closing_save:checked').val() == undefined){

        }else{
            post.close = $('#closing_save:checked').val();
        }
        post.account_id = $('#account_id').val();

        //alert(post.account_id);

        $.ajax({
            type: 'POST',
//            url: '<?//= site_url('Accounts/Link/updatelink/')?>//'+coaId+'/'+post.OBalance+'/'+post.Active+'/'+post.seprate+'/'+post.close,
            url: '<?= site_url('Accounts/Link/updatelink/')?>'+coaId+'/'+post.OBalance+'/'+post.Active+'/'+post.seprate+'/'+post.close,
            returnType: 'JSON',
            data: post,
            success: function (response) {
