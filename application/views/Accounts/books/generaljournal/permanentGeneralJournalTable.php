    ...
    foreach($transactions as $transaction):
        foreach($transaction as $transaction_2): ?>
            <tr class="odd gradeX">
                ...
                <td> <?php $rights = $this->AccessModel->check_access($_SESSION['user'][0]->id,33);
                ...
                <td><?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[5] == '1')){?>
                    <a target="_blank" href="<?= site_url('Accounts/Books/viewVoucher').'/'.$transaction_2->t_id.'/'.$this->uri->segment(5).'/'.$transaction->Permanent_VoucherNumber;?>" <button type="button"  data-id="<?= $transaction_2->t_id; ?>" class="btn btn-success " style="background-color: #517751;border-color: #517751;"><i class="fa fa-print"></i>
                    </button></a><?php }?>
                </td>
            </tr>
        <?php endforeach;
