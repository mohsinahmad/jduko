<?php if(isset($type)):
    foreach($transactions as $transaction):
        foreach($transaction as $transaction_2): ?>
            ...
            if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[0] == '1')){?>
                ...
                <td><?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[0] == '1')){?>
                    <a target="_blank" href="<?= site_url('AccountsAmountDescription').'/'.$transaction_2->t_id.'/'.$this->uri->segment(5).'/'.$vouch_type;?>"" <button type="button"  data-id="<?= $transaction_2->t_id; ?>" class="btn btn-success" style="background-color: #517751;border-color: #517751;"><i class="fa fa-money"></i>
                    </button></a>
                <?php }?>
                </td>
                <td>
                <?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[2] == '1')){?>
                    <button type="button"  data-id="<?= $transaction_2->t_id; ?>"  data-toggle="modal" data-target="#copy" class="btn btn-success copy_inc_transaction" style="background-color: #517751;border-color: #517751;"><i class="glyphicon glyphicon-share-alt"></i>
                    </button><?php }?>
                </td>
                <td><?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[5] == '1')){?>
                    <a target="_blank" href="<?= site_url('Accounts/Books/viewIncVoucher').'/'.$transaction_2->t_id.'/'.$this->uri->segment(5).'/'.$transaction_2->VoucherDateG;?>"> <button type="button"  data-id="<?= $transaction_2->t_id; ?>" class="btn btn-success " style="background-color: #517751;border-color: #517751;"><i class="fa fa-print"></i>
                    </button></a> <?php }?>
                </td>
            </tr>
        <?php endforeach;
    ...
    foreach($transactions as $transaction): ?>
        <tr class="odd gradeX">
            ...
            <td>
                <?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[0] == '1')){?>
                    <a target="_blank" href="<?= site_url('Accounts/AmountDescription/index').'/'.$transaction->t_id?>"" <button type="button"  data-id="<?= $transaction->t_id; ?>" class="btn btn-success " style="background-color: #517751;border-color: #517751;"><i class="fa fa-money"></i>
                    </button></a>
                <?php }?>
            </td>
            <td> <?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[2] == '1')){?>
                <button type="button"  data-id="<?= $transaction->t_id; ?>"  data-toggle="modal" data-target="#copy" class="btn btn-success copy_inc_transaction" style="background-color: #517751;border-color: #517751;"><i class="glyphicon glyphicon-share-alt"></i>
                </button> <?php }?>
                
            </td>
            <td>
        <?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[5] == '1')){?>
                <a target="_blank" href="<?= site_url('Accounts/Books/viewIncVoucher').'/'.$transaction->t_id.'/'.$this->uri->segment(5).'/'.$transaction->VoucherDateG;?>" <button type="button"  data-id="<?= $transaction->t_id; ?>" class="btn btn-success " style="background-color: #517751;border-color: #517751;"><i class="fa fa-print"></i>
                </button></a> <?php }?>
            </td>
        </tr>
    <?php endforeach; 
