...
class ItemSetupModel extends CI_Model
{
    ...
    {
        $this->db->select('*,item_setup.Id as a_id,a.Name as p_name,b.Name as s_name');
        $this->db->from('item_setup');
        $this->db->join('item_categories a', 'item_setup.category_Id = a.id');
    ...
    public function updateItems()
    {
        ...
        if($this->db->affected_rows() > 0){
            ...
            foreach ($this->input->post('DonationType') as $keys => $item) {
                ...
                $this->db->set('category_id',$this->input->post('ItemCategory'));
                $this->db->set('unit_of_measure',$this->input->post('UnitOfMeasure'));
