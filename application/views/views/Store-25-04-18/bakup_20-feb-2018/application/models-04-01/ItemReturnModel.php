...
class ItemReturnModel extends CI_Model
{
    ...
    {
        $this->db->select('users.UserName,item_issue.Id,item_issue.Issued_DateG,item_issue.Issued_DateH,item_issue.Remarks, item_issue.Form_Number,item_issue.Status, item_issue.is_returned,item_issue.Receive_Slip_Id');        if($_SESSION['user'][0]->IsAdmin != '1') {            $this->db->where('users.UserName', $_SESSION['user'][0]->UserName);        }       $this->db->from('item_issue');       $this->db->join('users', 'item_issue.CreatedBy = users.id');        if ($is_direct != '') {            $this->db->where('item_issue.Receive_Slip_Id IS NOT NULL');        }        $this->db->where('item_issue.is_returned', 0);		$this-db->where('users.UserName',$_SESSION['user'][0]->UserName);        return $this->db->get()->result();
    }

    public function GetItemReturned()
    {
        $this->db->select('item_return.return_dateG, item_return.return_dateH, item_return.Id as r_ID,item_return.Item_Issue_Form_Id, item_return.level_id, company_structure.LevelName,item_return.Status');
