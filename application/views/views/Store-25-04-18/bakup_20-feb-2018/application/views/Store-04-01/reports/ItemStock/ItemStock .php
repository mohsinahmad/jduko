...
<div class="content">
    <div class="row" id="content">
        ...
        <div style="text-align: center;">
            ...
            <h2 style="margin-top: 0px;font-size: 20px">آئٹم اسٹاک</h2>
            <h4 style="font-size: 15px"><span><?= $to[0]->Qm_date?></span></h4>
                ...
                <div class="row two">
                    <div class="col-lg-12">
                        ...
                        <div class="panel-body">
                            <h4 style="float: left">ابتدائ بیلنس :<span><?= number_format($Opening_Quantity_Cal[$item_key][0][0])?></span></h4>
                            <!--                                <div>-->
                            ...
                            <div class="table-responsive" align="middle" style="width: 108%;text-align: center; margin-right: -5%;overflow-x: hidden;overflow-y: hidden;">
                                <table class="table-bordered">
                                    ...
                                    <tr>
                                        <th colspan="3">
                                            <?php if (isset($item_setup[$item_key][0][0]->code) && isset($item_setup[$item_key][0][0]->name)){?>
                                                ...
                                                <h5 style="float: left;margin-top: -6%;margin-left: 2%;"> پیمائش کی اکائی : <span><?= $item_setup[$item_key][0][0]->unit_of_measure?></span></h5>
                                            <?php }?>
                                        </th>
                                    ...
                                    <tr style="line-height: 243%;">
                                        <th style="text-align: center;width:5%;">فارم نمبر</th>
                                        <th style="text-align: center;width: 9%">تاریخ</th>
                                        ...
                                        <th style="text-align: center;width: 8%;">مقدار</th>
                                        <th style="text-align: center;width: 14%;">بلینس</th>
                                    </tr>
                                    </thead>
                                    <?php $cal_quantity = $per_head = $recive = $issue = $return = $balance = $balance_New = 0; $balance += $Opening_Quantity_Cal[$item_key][0][0];?>
                                    <tbody>
                                    <?php foreach ($items[0] as $key => $item) {?>
                                        <tr>
                                            ...
                                            <?php if(isset($item->Issue_Quantity)){?>
                                                <td><?php echo number_format($item->Issue_Quantity);
                                                    $issue = $item->Issue_Quantity;
                                                    ?></td>
                                            <?php } else{?>
                                                <td></td>
                                            <?php }?>
                                            <?php $balance = ($balance + $item->recive_quantity + $item->return_quantity) - $item->Issue_Quantity; ?>
                                            <?php if ($balance < 0){
                                                $balance_New = $balance * -1; ?>
                                                <td>(<?php echo number_format($balance_New)?>)</td>
                                            <?php }else{?>
