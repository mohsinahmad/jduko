<?php if(isset($type)):
        ...
        foreach($transaction as $transaction_2): ?>
            ...
            if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[0] == '1')){?>
                ...
                <td><?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[5] == '1')){?>
                    <a target="_blank" href="<?= site_url('Accounts/Books/viewIncVoucher').'/'.$transaction_2->t_id.'/'.$this->uri->segment(5);?>"> <button type="button"  data-id="<?= $transaction_2->t_id; ?>" class="btn btn-success " style="background-color: #517751;border-color: #517751;"><i class="fa fa-print"></i>
                    </button></a> <?php }?>
                </td>
            </tr>
        <?php endforeach;
    ...
    foreach($transactions as $transaction): ?>
        ...
        <?php if($_SESSION['user'][0]->IsAdmin == 1 || ($rights != array() && $rights[0]->Rights[5] == '1')){?>
                <a target="_blank" href="<?= site_url('Accounts/Books/viewIncVoucher').'/'.$transaction->t_id.'/'.$this->uri->segment(5);?>" <button type="button"  data-id="<?= $transaction->t_id; ?>" class="btn btn-success " style="background-color: #517751;border-color: #517751;"><i class="fa fa-print"></i>
                </button></a> <?php }?>
            </td>
        </tr>
    <?php endforeach;
