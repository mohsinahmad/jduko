...
<div class="content" style="width: 95%;">
    ...
    <?php foreach ($AccName as $A_key => $acc){?>
        <div class="level">
            ...
            <div>
                <?php $trans_count = 0; $count = 1; $total_final_p = 0; $total_credit_final_p = 0 ; $total_debit_final_p = 0;
                        ...
                        <div class="col-lg-12" style="padding-right: 0px">
                            <div class="panel-body"> <?php if (isset($openingbalance[$A_key])){ $openingbalances = $openingbalance[$A_key];}?>
                                <?php if ($openingbalances < 0){
                                    ...
                                    <h4 style="margin-top: 0px;float:left;direction: ltr">(<?= number_format($new_openingbalance); ?>)<span>: ابتدائ بیلنس</span></h4>
                                <?php }else{ ?>
                                    <h4 style="margin-top: 0px;float:left;direction: ltr"><span><?= number_format($openingbalances); ?></span><span>: ابتدائ بیلنس</span></h4>
                                <?php }?>
                                <h4 style="margin-top: 0px;font-weight: normal;"><?=($Head[$A_key][0]->AccountName);?> :<span style="font-weight: normal;"><?=($AccName[$A_key][0]->AccountName);?></span>-<?=($AccName[$A_key][0]->AccountCode);?></h4>
                                <div class="table-responsive" align="middle" style="overflow-x: hidden;overflow-y: hidden;">
                                    <table class="table-bordered" style="width: 100%" id="debitp<?= $count?>">
                                        <thead>
                                        <tr>
                                        ...
                                        <tbody>
                                        <?php $balance = 0; $balance += $openingbalances; $total_debit = 0; $total_credit = 0; $TransactionCounts = 0; //print_r($transaction1);
                                        foreach($transaction1 as $Tkey => $transaction):?>
                                            <?php if($transaction->Permanent_VoucherNumber != ''){ $TransactionCounts++;?>
                                                <tr class="odd gradeX" style="text-align: center;">
                                                    ...
                                                    <?php if (isset($consolidated)){?>
                                                        <th style="width: 10%;text-align: center"><?= $transaction->LevelName?></th>
                                                    <?php }?>
                                    ...
                                    <?php if (isset($all)){?>
                                        <p>عارضی واؤچر</p>
                                        <table class="table-bordered" style="width: 100%" id="temp<?= $count;?>">
                                            <tbody>
                                            ...
                                            <tr class="odd gradeX" style="display: none">
                                                <?php }?>
                                                <td style="padding-right: 10%;width:10%;text-align:center"></td>
