...
class BookModel extends CI_Model
{
    ...
    public function get_all_transactions($book_type = '', $Level_id = '')
    {
        if ($this->year_status->Active == 1) {
            ...
            $where = "transactions.LevelID = '$Level_id' and transactions.VoucherDateG >= ( CURDATE() - INTERVAL 9 DAY )";
            $this->db->where($where);
            if ($book_type != '') {
    ...
    {
        if ($this->year_status->Active == 1) {
            $this->db->select('SUM(transactions.Debit)as debit,transactions.Seprate_series_num,transactions.Id as t_id,transactions.VoucherType, transactions.Permanent_VoucherNumber, transactions.VoucherNo, transactions.VoucherDateG, transactions.VoucherDateH, transactions.PaidTo, departments.DepartmentName, transactions.Remarks, transactions.Seprate_series_num');
            $this->db->from('transactions');
            $this->db->join('company_structure', 'company_structure.id = transactions.LevelID');
            $this->db->join('account_title', 'account_title.id = transactions.AccountID');
...
//            $this->db->like('transactions.VoucherDateH', $this->session->userdata('current_year'),'after');
            ...
            $this->db->select('SUM(archived_transactions.Debit)as debit,transactions.Seprate_series_num,archived_transactions.Id as t_id,archived_transactions.VoucherType, archived_transactions.Permanent_VoucherNumber, archived_transactions.VoucherNo, archived_transactions.VoucherDateG, archived_transactions.VoucherDateH, archived_transactions.PaidTo, departments.DepartmentName, archived_transactions.Remarks');
            $this->db->from('archived_transactions');
            $this->db->join('company_structure', 'company_structure.id = archived_transactions.LevelID');
            $this->db->join('account_title', 'account_title.id = archived_transactions.AccountID');
...
//            $this->db->like('transactions.VoucherDateH', $this->session->userdata('current_year'));
    ...
    public function get_edit_transaction($id, $company_id)
    {
        ...
        if ($this->year_status->Active == 1) {
            $this->db->select('transactions.TaxDebit,transactions.Seprate_series_num,transactions.IsReverse,transactions.LinkID,transactions.CreatedOn,transactions.Createdby,transactions.Permanent_VoucherDateH, transactions.Permanent_VoucherDateG, transactions.Permanent_VoucherNumber, transactions.Id as t_id,company_structure.id as level_id,company_structure.LevelName,transactions.VoucherType,transactions.VoucherNo,transactions.VoucherDateG,transactions.VoucherDateH, departments.Id as DepartmentId,departments.DepartmentName,transactions.PaidTo,transactions.Remarks,transactions.Debit,transactions.Credit,transactions.Description,account_title.id as AccountID, account_title.AccountName, account_title.Type as AccountType, transactions.SequenceNo,transactions.ChequeNumber,transactions.ChequeDate');
            $this->db->from('transactions');
            $this->db->join('company_structure', 'company_structure.id = transactions.LevelID');
            $this->db->join('account_title', 'account_title.id = transactions.AccountID');
            $this->db->join('departments', 'departments.Id = transactions.DepartmentId', 'left');
    ...
    {
        if ($this->year_status->Active == 1) {
            $this->db->select('VoucherType,VoucherNo,LevelID,Seprate_series_num');
            $this->db->where('Id', $id);
            $this->db->from('transactions');
            $data = $this->db->get()->result();
    ...
    public function update_Permanent_Voucher($type, $TokeepVoucher, $level_id)
    {
        if ($type == 'cr') {
            foreach ($_POST['data']['toTemp'] as $key => $value) {
                $count = $this->get_max_tranCount($_POST['vouch_no']['VoucherNo'][$key], $level_id,$type);
...
//                    exit();
    ...
    public function get_permanent_trans_bydate($book_type, $id, $to, $from)
    {
        if ($this->year_status->Active == 1) {
    ...
    public function get_voucher($vouch_no, $vouch_type, $level_id, $Seprate_series_num)
    {
        ...
        if ($this->year_status->Active == 1) {
            ...
            `transactions`.`ChequeNumber`, `transactions`.`ChequeDate`, `company_structure`.`ParentCode` as `Comp_Parent`, `company_structure`.`LevelName`, `transactions`.`PaidTo`, `transactions`.`Remarks`, `transactions`.`Description`, `transactions`.`Debit`, `transactions`.`Credit`, `transactions`.`Permanent_VoucherNumber`, `transactions`.`Permanent_VoucherDateH`, `transactions`.`Permanent_VoucherDateG`, `a`.`AccountName`, `transactions`.`Createdby`, `transactions`.`CreatedOn`, `transactions`.`UpdatedBy`, `transactions`.`UpdatedOn`, `a`.`Type`, `transactions`.`count`, `departments`.`DepartmentName`, `a`.`AccountCode`, `transactions`.`VoucherType`, `transactions`.`VoucherNo`, `transactions`.`VoucherDateH`, `transactions`.`VoucherDateG`, `b`.`AccountName` as `ParentName` FROM `transactions` JOIN `company_structure` ON `company_structure`.`id` = `transactions`.`LevelID` JOIN `account_title` `a` ON `a`.`id` = `transactions`.`AccountID` LEFT JOIN `departments` ON `departments`.`id` = `transactions`.`DepartmentId` JOIN `account_title` `b` ON `b`.`AccountCode` = `a`.`ParentCode` WHERE `transactions`.`VoucherNo` = '$vouch_no' AND `transactions`.`LevelID` = '$level_id' AND `transactions`.`VoucherType` = '$vouch_type' AND `transactions`.`Seprate_series_num` =  '" . $Seprate_series_num . "' ORDER BY `amounttype` , transactions.SequenceNo");
        } else {
            $query = $this->db->query("SELECT 
            CASE WHEN archived_transactions.Debit > 0 THEN 1 ELSE 2 END AS amounttype,
            `archived_transactions`.`ChequeNumber`, `archived_transactions`.`ChequeDate`, `company_structure`.`ParentCode` as `Comp_Parent`, `company_structure`.`LevelName`, `archived_transactions`.`PaidTo`, `archived_transactions`.`Remarks`, `archived_transactions`.`Description`, `archived_transactions`.`Debit`, `archived_transactions`.`Credit`, `archived_transactions`.`Permanent_VoucherNumber`, `archived_transactions`.`Permanent_VoucherDateH`, `archived_transactions`.`Permanent_VoucherDateG`, `a`.`AccountName`, `archived_transactions`.`Createdby`, `archived_transactions`.`CreatedOn`, `archived_transactions`.`UpdatedBy`, `archived_transactions`.`UpdatedOn`, `a`.`Type`,`departments`.`DepartmentName`, `a`.`AccountCode`, `archived_transactions`.`VoucherType`, `archived_transactions`.`VoucherNo`, `archived_transactions`.`VoucherDateH`, `archived_transactions`.`VoucherDateG`, `b`.`AccountName` as `ParentName` FROM `archived_transactions` JOIN `company_structure` ON `company_structure`.`id` = `archived_transactions`.`LevelID` JOIN `account_title` `a` ON `a`.`id` = `archived_transactions`.`AccountID` LEFT JOIN `departments` ON `departments`.`id` = `archived_transactions`.`DepartmentId` JOIN `account_title` `b` ON `b`.`AccountCode` = `a`.`ParentCode` WHERE `archived_transactions`.`VoucherNo` = '$vouch_no' AND `archived_transactions`.`LevelID` = '$level_id' AND `archived_transactions.Year` = '$this->activeyear' AND `archived_transactions`.`VoucherType` = '$vouch_type' ORDER BY `amounttype` , `archived_transactions.SequenceNo`");
        }
        return $query->result();
    }

    public function get_book_Amount($voucherType, $VoucherNumber, $level, $Seprate_series_num)
    {
        //return $voucherType.$VoucherNumber.$level.'='.$Seprate_series_num;
...
//            echo '</pre>';
            ...
            $this->db->select('company_structure.LevelName,income.LevelID,income.AccountID,income.VoucherNo,income.Permanent_VoucherNumber, income.VoucherType, income.VoucherDateG,income.Permanent_VoucherDateG, income.VoucherDateH,income.Permanent_VoucherDateH, income.Remarks, income.Debit, income.Credit');
            $this->db->from('income');
...
//                $this->db->select('archived_transactions.LevelID,archived_transactions.AccountID,archived_transactions.VoucherNo,archived_transactions.Permanent_VoucherNumber,archived_transactions.VoucherType, archived_transactions.VoucherDateG,archived_transactions.Permanent_VoucherDateG, archived_transactions.VoucherDateH,archived_transactions.Permanent_VoucherDateH,archived_transactions.Remarks, archived_transactions.Debit,archived_transactions.Credit');
//                $this->db->from('archived_transactions');
//                $this->db->where_in('archived_transactions.LevelID', $Level_id[0]);
//                $this->db->where_in('archived_transactions.AccountID', $account);
//                if ($ledgerof == 'p') {
...
//                $trans_query = $this->db->get_compiled_select();
//
//                $this->db->select('archived_income.LevelID,archived_income.AccountID,archived_income.VoucherNo,archived_income.Permanent_VoucherNumber, archived_income.VoucherType, archived_income.VoucherDateG,archived_income.Permanent_VoucherDateG, archived_income.VoucherDateH,archived_income.Permanent_VoucherDateH, archived_income.Remarks, archived_income.Debit, archived_income.Credit');
//                $this->db->from('archived_income');
//                $this->db->where_in('archived_income.LevelID', $Level_id[0]);
//                $this->db->where_in('archived_income.AccountID', $account);
//                if ($ledgerof == 'p') {
...
//                $income_query = $this->db->get_compiled_select();
//
//                $query = $this->db->query('Select `LevelID`, `AccountID`, `VoucherNo`, `Permanent_VoucherNumber`, `VoucherType`, `VoucherDateG`, `Permanent_VoucherDateG`, `VoucherDateH`, `Permanent_VoucherDateH`, `Remarks`, `Debit`, `Credit` From(' . $trans_query . ' UNION ' . $income_query . ') as t');
//                $result[] = $query->result();
//            }
        }
        return $result;
...
//            foreach ($accounts as $account) {
//                $this->db->select('LevelID , AccountID ,SUM(Debit) as tdebit, SUM(Credit) as tcredit');
//                $this->db->from('archived_transactions');
